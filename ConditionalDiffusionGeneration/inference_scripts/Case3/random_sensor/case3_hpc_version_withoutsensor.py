...
#true_measurement = true_measurement[:, :, :1]  # This ensures (384, 10, 2)
...
#    device=device,
#    name='case3',
#    coords_path=f'input_case3/random_sensor/{no_of_sensors}/coords.npy',
#    max_val=np.load("input_case3/data_scale/data_max.npy"),
#    min_val=np.load("input_case3/data_scale/data_min.npy"),
#    normalizer_params_path="input_case3/cnf_model/normalizer_params.pt",
#    ckpt_path="input_case3/cnf_model/checkpoint_4800.pt",
#    batch_size=384
#)
...
sample_fn = partial(sampler.p_sample_loop, model=u_net_model, measurement_cond_fn=None)
...
samples = [sample_fn(x_start=x_start[i:i+1], record=False, save_root=None) for i in range(x_start.shape[0])]
...
#orch.save(operator, f"{output_dir}/operator.pt")
#torch.save(samples, f"{output_dir}/samples.pt")
np.save(f"{output_dir}/pred_data_list.npy", gen_fields.cpu().numpy())
